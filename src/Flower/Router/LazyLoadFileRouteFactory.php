 ...
 */
class LazyLoadFileRouteFactory implements FactoryInterface, MutableCreationOptionsInterface {
    ...
    public function createService(ServiceLocatorInterface $serviceLocator)
    {
        ...
        if (!isset($options['name'])) {
            throw new RuntimeException('LazyLoadResourceRoute needs name option');
        }
        $name = $options['name'];
        unset ($options['name']);
        $specs = array_merge($options, $this->getFileService()->read($name));
